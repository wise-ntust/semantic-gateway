...
延遲軸也量了（sender 端 head 運算 vs receiver 端 tail 運算，每 clip）：

| split | sender 運算 | receiver 運算 | feature bytes |
|---|---:|---:|---:|
| frame path | 0 ms | 64.6 ms（整個模型） | 24 KB |
| feature @ layer2 | 50.1 ms | 10.5 ms | 803 KB |
| feature @ layer4 | 64.1 ms | 0.0 ms | 50 KB |
| feature @ avgpool | 63.6 ms | 0.0 ms | 520 B |

> [!IMPORTANT]
> ***sender 運算隨 split 加深單調上升（stem 1.8 ms 到 layer4 64 ms）。唯一讓 feature 比 frame 小的 avgpool，sender 要做 64 ms（等於跑完整個模型）。沒有任何 split 是「bytes 小」又「sender 運算低」，兩個軸得到同一個結論。（此為 sandbox CPU 相對計時，證明趨勢；GPU 絕對值待 king 回線補測。）***
...
> - **RQ3 只測 int8**：沒測 learned / entropy coding 的 feature 壓縮，crossover 可能存在只是本研究沒測到。「深 split 會加 sender latency」已用 head/tail 計時實測（sandbox CPU 相對值），但 GPU 絕對延遲與真實 edge 硬體尚未量。
